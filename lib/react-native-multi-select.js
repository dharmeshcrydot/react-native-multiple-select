...
export default class MultiSelect extends Component {
        ...
        <View
          style={[
            styles.selectedItem,
            {
              ...
              //               height: 40,
              // =======
              width: item[displayKey].length * 6 + 60,
              justifyContent: 'center',
              height: 25,
  ...
  _removeItem = item => {
    const { uniqueKey, selectedItems, onSelectedItemsChange, quantityChange } = this.props;
    ...
    quantityChange(text = undefined, "total");
  };
  ...
  _clearSelectorCallback = () => {
    const { onClearSelector, quantityChange } = this.props;
    this._clearSelector();
    if (onClearSelector) {
      onClearSelector();
      quantityChange(text = undefined, "total")
  ...
  _toggleSelector = () => {
    const { onToggleList, quantityChange } = this.props;
    quantityChange(text = undefined, "total")
    this.setState({
  ...
  updateQuantity(value, item) {
    item.isQuantity = value;
  }

  _getRow = item => {
    const { submitButtonColor, selectedItemIconColor, displayKey, styleRowList, quantityChange, onSelectedItemsChange } = this.props;
    return (
      <TouchableOpacity
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            {this._itemSelected(item) ? (
              ...
              style={{
                ...
                borderColor: item.disabled ? 'red' : 'grey',
                fontSize: 20,
                color: item.disabled ? 'red' : 'transparent',
              }}
            />}
            ...
            <Text
              style={[
                ...
                item.disabled ? { color: 'red' } : { color: 'grey' }
              ]}
            >
              {item[displayKey]}
            </Text>
            {/* {item && item.disabled ? "" : ""
              //  <TextInput style={{ height: 45, padding: 15, borderColor: 'black', borderWidth: 1, margin: 15, borderRadius: 5, }}
            } */}
            {
              !item.disabled ? <NumericInput
                minValue={0}
                maxValue={item.quantity}
                initValue={item.isQuantity.toString()}
                value={item.isQuantity.toString()}
                onChange={value => { this.updateQuantity(value, item), quantityChange(item) }}
                ...
                rightButtonBackgroundColor={submitButtonColor}
                leftButtonBackgroundColor={submitButtonColor} /> : <View />
            }
          </View >
        </View >
      </TouchableOpacity >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Text
            style={[
              ...
              this._itemStyle(item),
              item.disabled ? { color: 'grey' } : { color: 'red' }
            ]}
          >
